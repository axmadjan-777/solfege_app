...
#### Шаблон письма «Confirm signup» (обязательно для авто-подтверждения)

Authentication → **Emails / Templates → Confirm signup**. Ссылка должна вести на
приложение с `token_hash`, тогда подтверждение срабатывает в любом браузере и на
любом устройстве (приложение само вызывает `verifyOTP`):

```html
<a href="{{ .SiteURL }}?token_hash={{ .TokenHash }}&type=email">Подтвердить email</a>
```

Если оставить шаблон со ссылкой `?code=...` (PKCE), подтверждение сработает
только в том же браузере, где шла регистрация.
...
3. Пользователь открывает ссылку из письма. Приложение на старте разбирает адрес
   (`AuthService.handleEmailConfirmationLink`): по `token_hash` вызывает `verifyOTP`
   и создаёт сессию **автоматически** — экран «Подтвердите email» при этом не
   показывается, сразу идёт **«Подтверждаем почту…»** → профиль → успех.
